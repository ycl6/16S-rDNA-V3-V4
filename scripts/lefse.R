...
lefse_1name_obj = function(ps, group) {
	...
	colnames(tax) = c("name", as.vector(sample_data(ps)$Sample_ID))
	tax = rbind(tax, c("subject", as.vector(sample_data(ps)$Sample_ID)))
	return(tax)
}
...
lefse_obj = function(ps) {
	...
	tax1 = tax1[order(tax1$Sample_ID),]
	tax1 = reshape2::dcast(tax1, Kingdom~Sample_ID, value.var = "Abundance")
	names(tax1)[1] = "name"
	...
	tax2 = tax2[order(tax2$Sample_ID, tax2$name),]
	tax2 = reshape2::dcast(tax2, name~Sample_ID, value.var = "Abundance")
	...
	tax3 = tax3[order(tax3$Sample_ID, tax3$name),]
	tax3 = reshape2::dcast(tax3, name~Sample_ID, value.var = "Abundance")
	...
	tax4 = tax4[order(tax4$Sample_ID, tax4$name),]
	tax4 = reshape2::dcast(tax4, name~Sample_ID, value.var = "Abundance")
	...
	tax5 = tax5[order(tax5$Sample_ID, tax5$name),]
	tax5 = reshape2::dcast(tax5, name~Sample_ID, value.var = "Abundance")
	...
	tax6 = tax6[order(tax6$Sample_ID, tax6$name),]
	tax6 = reshape2::dcast(tax6, name~Sample_ID, value.var = "Abundance")
